...
class RecordSerializer(serializers.ModelSerializer):
    ...
    def get_files(self,obj):
      ...
      return [  {'id':file.id, 'url': f'{settings.SITE_URL}{file.file.url}'} for file in files ]
    ...
    def update(self, instance, validated_data):
        ...
        if 'delete_files' in self.context['request'].data:
             delete_files = self.context['request'].data.getlist('delete_files')
             File.objects.filter(records_FK=instance, id__in=delete_files).delete()
