           ...
           const response = await axios.get(`/api/records/${recordId.value}/`);
            ...
            subject_id.value = record.value.subject.id;
             if (record.value.files && record.value.files.length > 0) {
                 oldFiles.value = record.value.files.map( (file) => {
                      let preview= null
                      if(file.url.endsWith('.xlsx')){
                         preview="/icon-file-xlsx.png"
                      }
                      else if(file.url.endsWith('.pdf')){
                        preview="/icon-file-pdf.png"
                      }
                     else if (file.url.endsWith('.docx') || file.url.endsWith('.doc')) {
                     ...
                     return  {
                         ...
                         id: file.id,
                         isOld: true,
                         file: { name: file.url.split('/').pop(), url: file.url }
                      }
                });
...
async function removeFile(index, item) {
    if(item.isOld && item.id){
        filesToDelete.value.push(item.id)
    }
...
async function addRecord() {
    ...
    formData.append('is_published', record.value.is_published == true );
    ...
    for (let i = 0; i < filesToDelete.value.length; i++) {
        formData.append('delete_files', filesToDelete.value[i]);
    }
                      ...
                      <ul v-if="selectedFiles.length" class="file-list">
                           <li v-for="(item, index) in selectedFiles" :key="item.id" class="file-item">
                               <a :href="item.file.url" download>
                                 <button type="button" class="remove-file-button" @click.prevent="removeFile(index, item)">
